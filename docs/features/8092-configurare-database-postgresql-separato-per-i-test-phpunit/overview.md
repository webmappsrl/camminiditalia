...
- **Bypass silenzioso della fix via `config:cache`**: se in futuro qualcuno esegue `php artisan config:cache` nel container di sviluppo, Laravel salta il caricamento di `.env`/`.env.testing` e usa la config già cacheata (potenzialmente ancora puntata al DB di sviluppo) — la fix smetterebbe di avere effetto senza alcun errore visibile, riproducendo silenziosamente l'incidente originale. Mitigazione: nessuna azione automatica in questo ciclo (accettato come rischio noto); valutare in futuro un check esplicito o una nota di attenzione in `CLAUDE.md`.
